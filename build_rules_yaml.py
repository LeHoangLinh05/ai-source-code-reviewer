...
def R(id, week, req, ctype, prio, ai=False, hint=None):
    _rules.append(dict(id=id, week=week, req=req, ctype=ctype, prio=prio, ai=ai, hint=hint))
...
R("RC-W1-08", 1, "Tách thư mục utils/ (jwt_handler, password_hash)", "required_folder", "P1")
R("RC-W1-09", 1, "Endpoint /register (password phải hash)", "required_code_pattern", "P0")
R("RC-W1-10", 1, "Endpoint /login trả access+refresh token", "required_code_pattern", "P0", True,
...
R("RC-W1-11", 1, "Endpoint /token/refresh", "required_code_pattern", "P0", True,
...
R("RC-W1-13", 1, "Có logout / vô hiệu hoá token", "required_code_pattern", "P1", True,
...
R("RC-W1-17", 1, "Password được hash trước khi lưu DB", "required_dependency", "P0", True,
...
R("RC-W2-06", 2, "Compose có service DB (MySQL/Mongo/Redis)", "required_config_key", "P0")
...
R("RC-W2-12", 2, "README có link demo triển khai (Render/Railway/EC2)", "required_config_key", "P2")
...
R("RC-W3-06", 3, "Cache GET /products với TTL", "required_code_pattern", "P0", True,
...
R("RC-W3-07", 3, "Cache bị xoá khi có CRUD thay đổi dữ liệu", "required_code_pattern", "P0", True,
  "Xác nhận cache bị xoá NGAY khi tạo/sửa/xoá sản phẩm — không phải chỉ hết hạn tự nhiên "
  "theo TTL.")
R("RC-W3-08", 3, "Có RBAC (Role, Permission, Role_Permission)", "required_code_pattern", "P1")
R("RC-W3-09", 3, "Có job sync định kỳ MySQL → MongoDB", "required_code_pattern", "P0", True,
...
R("RC-W3-10", 3, "Lưu lịch sử sync vào collection sync_logs", "required_code_pattern", "P1")
R("RC-W3-11", 3, "Alembic có revision loại create table VÀ alter table", "required_code_pattern", "P1")
...
R("RC-W4-05", 4, "Có trang Login + Protected Route", "required_code_pattern", "P0", True,
...
R("RC-W4-09", 4, "Bảng dữ liệu lớn: pagination/infinite/virtual scroll", "required_dependency", "P1")
...
# --- Tuần 5 — Realtime ---
R("RC-W5-01", 5, "Có SSE hoặc WebSocket, broadcast nhiều user", "required_code_pattern", "P0", True,
  "Xác nhận broadcast được tới NHIỀU client đồng thời, không phải chỉ echo 1-1 giữa 1 client "
  "và server.")
R("RC-W5-02", 5, "Có Connection Manager quản lý nhiều client", "required_code_pattern", "P0", True,
  "Xác nhận có cấu trúc lưu danh sách connections (dict/list theo room hoặc user) — không "
  "phải 1 biến global single-connection.")
R("RC-W5-03", 5, "JWT được validate khi connect realtime", "required_code_pattern", "P0", True,
...
R("RC-W5-04", 5, "Redis Pub/Sub đồng bộ multi-instance", "required_code_pattern", "P1", True,
...
R("RC-W5-06", 5, "Frontend có hook/client kết nối realtime", "required_code_pattern", "P1")
R("RC-W5-07", 5, "(Tự học) WebRTC video call", "required_code_pattern", "P2")

# --- Tuần 6 — AI Chatbox với Context Memory ---
R("RC-W6-01", 6, "Lịch sử hội thoại lưu theo user_id/session_id", "required_code_pattern", "P0")
R("RC-W6-02", 6, "Có cơ chế memory (buffer/summary)", "required_code_pattern", "P0", True,
  ...
  "không giới hạn.")
R("RC-W6-03", 6, "Có tool/function calling schema cho AI", "required_code_pattern", "P1")
R("RC-W6-04", 6, "Có endpoint reset/clear session", "required_code_pattern", "P1", True,
...
R("RC-W6-07", 6, "UI phân biệt AI vs User message + hiệu ứng loading/typing", "required_code_pattern", "P2")
...
R("RC-W7-04", 7, "Có pipeline retrieve → LLM → response", "required_code_pattern", "P0", True,
...
R("RC-W7-05", 7, "Top-K / similarity threshold cấu hình được", "required_code_pattern", "P1")
R("RC-W7-06", 7, "Kết hợp Memory + RAG", "required_code_pattern", "P1", True,
...
R("RC-W7-07", 7, "Có debug mode xem context + RAG result", "required_code_pattern", "P2")
...
R("RC-GEN-02", "GEN", "Có .gitignore loại trừ .env, node_modules/, venv/", "required_config_key", "P1")
R("RC-GEN-03", "GEN", "Kết hợp ≥ 2/3 DB (SQL/Mongo/Redis) — bonus final project", "required_config_key", "P2")
R("RC-GEN-04", "GEN", "Có CI cơ bản", "required_folder", "P2")
R("RC-GEN-05", "GEN", "docker-compose up chạy toàn bộ stack 1 lệnh", "required_config_key", "P1")
...
TARGETS = {
    ...
    "RC-W1-04": {"manifest_glob": list(PY_MANIFEST), "package_any_of": ["pyjwt", "python-jose"]},
    "RC-W1-05": {"glob": ["**/routers/"]},
    "RC-W1-06": {"glob": ["**/schemas/"]},
    "RC-W1-07": {"glob": ["**/models/"]},
    "RC-W1-08": {"glob": ["**/utils/"]},
    "RC-W1-09": {"glob": list(PY_GLOB), "regex": r"/register", "min_matches": 1},
    "RC-W1-10": {"glob": list(PY_GLOB), "regex_all_of": [r"/login", r"refresh_token"], "min_matches": 1},
    "RC-W1-11": {"glob": list(PY_GLOB), "regex": r"/token/refresh|/refresh", "min_matches": 1},
    "RC-W1-12": {"glob": list(PY_GLOB), "regex": r"/users/me", "min_matches": 1},
    "RC-W1-13": {"glob": list(PY_GLOB), "regex": r"/logout|blacklist", "min_matches": 1},
    "RC-W1-14": {"glob": list(PY_GLOB), "regex": r"^import logging|loguru", "min_matches": 1},
    "RC-W1-15": {"glob": list(PY_GLOB), "regex": r"HTTPException|exception_handler", "min_matches": 1},
    "RC-W1-16": {"glob": ["**/README.md"]},
    "RC-W1-17": {"manifest_glob": list(PY_MANIFEST), "package_any_of": ["bcrypt", "passlib", "argon2-cffi"]},
    ...
    "RC-W2-05": {"glob_any_of": ["docker-compose.yml", "docker-compose.yaml", "docker/docker-compose.yml"]},
    "RC-W2-06": {"file_glob": ["docker-compose.yml", "docker-compose.yaml"], "key_pattern": r"mysql|postgres|mongo|redis"},
    "RC-W2-07": {"glob": ["**/nginx/nginx.conf", "**/nginx.conf"]},
    "RC-W2-08": {"glob": [".env"]},
    "RC-W2-09": {"glob": ["**/.env.example"]},
    "RC-W2-10": {"file_glob": ["docker-compose.yml", "docker-compose.yaml"], "key_pattern": r"healthcheck:"},
    "RC-W2-11": {"file_glob": ["**/nginx.conf"], "key_pattern": r"ssl_certificate"},
    "RC-W2-12": {"file_glob": ["**/README.md"], "key_pattern": r"https?://"},
    ...
    "RC-W3-04": {"manifest_glob": list(PY_MANIFEST), "package_any_of": ["pymongo", "motor"]},
    "RC-W3-05": {"manifest_glob": list(PY_MANIFEST), "package_any_of": ["redis", "aioredis"]},
    "RC-W3-06": {"glob": list(PY_GLOB), "regex": r"\.expire\(|setex\(|ttl=", "min_matches": 1},
    "RC-W3-07": {"glob": list(PY_GLOB), "regex": r"cache\.delete|invalidate_cache|cache_delete", "min_matches": 1},
    "RC-W3-08": {"glob": list(PY_GLOB), "regex_all_of": [r"class Role", r"class Permission"], "min_matches": 1},
    "RC-W3-09": {"glob": ["**/workers/**/*.py", "**/services/**/*.py"], "regex": r"sync", "min_matches": 1},
    "RC-W3-10": {"glob": list(PY_GLOB), "regex": r"sync_logs", "min_matches": 1},
    "RC-W3-11": {"glob": ["**/alembic/versions/*.py"], "regex_all_of": [r"op\.create_table", r"op\.alter_column|op\.add_column"], "min_matches": 1},
    "RC-W3-12": {"glob": list(PY_GLOB), "regex": r"benchmark|query_time|elapsed_time", "min_matches": 1},

    "RC-W4-01": {"manifest_glob": list(JS_MANIFEST), "package_any_of": ["next"], "version_constraint": "^15"},
    ...
    "RC-W4-05": {"glob": ["**/middleware.ts", "**/middleware.js"]},
    "RC-W4-06": {"glob": ["**/tsconfig.json"]},
    "RC-W4-07": {"manifest_glob": list(JS_MANIFEST), "package_any_of": ["recharts", "chart.js"]},
    "RC-W4-08": {"manifest_glob": list(JS_MANIFEST), "package_any_of": ["mermaid"]},
    "RC-W4-09": {"manifest_glob": list(JS_MANIFEST), "package_any_of": ["@tanstack/react-table", "react-window", "react-virtualized"]},
    "RC-W4-10": {"glob": list(JS_GLOB), "regex": r"(?i)skeleton", "min_matches": 1},
    "RC-W4-11": {"glob_any_of": ["**/.env.local", "**/.env.local.example"]},

    "RC-W5-01": {"glob": list(PY_GLOB), "regex": r"websocket|WebSocket|text/event-stream|EventSourceResponse", "min_matches": 1},
    "RC-W5-02": {"glob": list(PY_GLOB), "regex": r"ConnectionManager", "min_matches": 1},
    "RC-W5-03": {"glob": list(PY_GLOB), "regex": r"websocket|WebSocket", "min_matches": 1},
    "RC-W5-04": {"glob": list(PY_GLOB), "regex_all_of": [r"\.publish\(", r"\.subscribe\("], "min_matches": 1},
    "RC-W5-05": {"glob": list(PY_GLOB), "regex": r"class Message|class ChatLog|chat_history", "min_matches": 1},
    "RC-W5-06": {"glob": list(JS_GLOB), "regex": r"useWebSocket|EventSource|WebSocket\(", "min_matches": 1},
    "RC-W5-07": {"glob": list(JS_GLOB), "regex": r"RTCPeerConnection", "min_matches": 1},

    "RC-W6-01": {"glob": list(PY_GLOB), "regex_all_of": [r"chat_history|conversation", r"user_id"], "min_matches": 1},
    "RC-W6-02": {"glob": list(PY_GLOB), "regex": r"Memory|ConversationBuffer|conversation_summary", "min_matches": 1},
    "RC-W6-03": {"glob": list(PY_GLOB), "regex": r"tools=|function_call", "min_matches": 1},
    "RC-W6-04": {"glob": list(PY_GLOB), "regex": r"/reset|/clear", "min_matches": 1},
    "RC-W6-05": {"glob": list(JS_GLOB), "regex": r"WebSocket|EventSource", "min_matches": 1},
    "RC-W6-06": {"glob": list(PY_GLOB), "regex": r"logging\.|logger\.", "min_matches": 1},
    "RC-W6-07": {"glob": list(JS_GLOB), "regex": r"role\s*===?\s*['\"](assistant|user)['\"]", "min_matches": 1},

    "RC-W7-01": {"manifest_glob": list(PY_MANIFEST), "package_any_of": ["chromadb", "faiss-cpu", "qdrant-client"]},
    "RC-W7-02": {"glob": list(AI_GLOB), "regex": r"embed|Embeddings", "min_matches": 1},
    "RC-W7-03": {"glob": list(AI_GLOB), "regex": r"TextSplitter|chunk", "min_matches": 1},
    "RC-W7-04": {"glob": list(AI_GLOB), "regex": r"retriev", "min_matches": 1},
    "RC-W7-05": {"glob": list(AI_GLOB), "regex": r"top_k|similarity_threshold", "min_matches": 1},
    "RC-W7-06": {"glob": list(AI_GLOB), "regex_all_of": [r"Memory|conversation", r"retriev"], "min_matches": 1},
    "RC-W7-07": {"glob": list(AI_GLOB), "regex": r"retrieved_chunks|debug.*context", "min_matches": 1},
    "RC-W7-08": {"glob": list(AI_GLOB), "regex": r"query_hash|cache_key", "min_matches": 1},

    "RC-GEN-01": {"glob_any_of": ["**/requirements.txt", "**/pyproject.toml"]},
    "RC-GEN-02": {"file_glob": [".gitignore"], "key_pattern": r"\.env"},
    "RC-GEN-03": {"file_glob": ["docker-compose.yml", "docker-compose.yaml"], "key_pattern": r"(mysql|postgres).*(mongo).*(redis)|redis.*mongo"},
    "RC-GEN-04": {"glob": [".github/workflows/"]},
    "RC-GEN-05": {"file_glob": ["docker-compose.yml", "docker-compose.yaml"], "key_pattern": r"services:"},
}
...
def validate(output_rules):
    ...
    for r in output_rules:
        if r["needs_ai_verification"]:
            assert "ai_hint" in r, f"{r['rule_id']}: needs_ai_verification=true nhưng thiếu ai_hint"
        else:
            assert "ai_hint" not in r, f"{r['rule_id']}: needs_ai_verification=false nhưng có ai_hint thừa"
