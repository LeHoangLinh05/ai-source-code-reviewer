...
**Module này LÀM:**
- Agent ReAct tool-calling (5 tools cố định)
...
```text
backend/app/ai/
├── agent.py                  # Main AI Agent — ReAct loop (mục 3)
├── prompts.py                 # REVIEW_SYSTEM_PROMPT (mục 6, copy nguyên văn)
├── tools/
...
│   └── openai_client.py       # OpenAIClient (fallback)
└── rag/
...
google-genai              # Gemini SDK, tool calling native
openai                    # Fallback LLM
chromadb                  # Vector DB
...
**Quyết định: KHÔNG dùng LangChain.** Direct SDK (google-genai / openai) — ít abstraction,
debug tool calling dễ hơn, ít dependency hơn cho 5 tools đơn giản.
...
## 4. Năm Tools — JSON Schema cho Function Calling

> Contract cố định — **không đổi tên field hoặc enum**, frontend `/ai-debug` và DB schema
> phụ thuộc trực tiếp vào tên field dưới đây.
...
| 3 | Structured Output | Tool calling enforcement — reject bất kỳ response nào không phải tool call |
...
| 6 | Hard Limit | `agent.py`: đếm tool call trong loop, dừng cứng ở 20 — **code**, không phải prompt |
...
Client:      google-genai SDK / openai SDK — trực tiếp, KHÔNG qua LangChain
Retry:       Exponential backoff (max 3 retries, base 2s)
Config:      API key từ .env qua core/config.py
Abstraction: class LLMClient (base) → GeminiClient, OpenAIClient
```
...
class LLMClient(ABC):
    ...
    async def generate_with_tools(
        self, system_prompt: str, messages: list[Message], tools: list[dict],
    ) -> LLMResponse: ...
```
...
- [ ] LLMClient.generate_with_tools() chạy được với Gemini, tool calling nhận đúng schema
- [ ] Cả 5 tool declare đúng JSON schema ở mục 4, tên field khớp 100% với output contract
- [ ] agent.py: đếm tool call, dừng cứng ở 20 (test bằng cách mock loop vô hạn)
...
- [ ] Agent xử lý được roadmap_verification_queue: đọc file, tạo issue category != requirement
      nếu phát hiện sai, không tạo gì nếu đúng
- [ ] Agent KHÔNG re-report item đã có trong roadmap_compliance_summary
- [ ] Tool call logs ghi đủ vào MongoDB, có duration_ms
